...
int ThrottleManager::getDisplaySpeed(int throttle) const {
    if (speedDisplayAsPercent) {
        ...
        speed = speed / 126 *100;
        int iSpeed = speed; if (iSpeed-speed >= 0.5) iSpeed++;
        return iSpeed;
...
void ThrottleManager::changeDirection(int throttle, Direction direction) {
    ...
    if (locoCount == 1) {
        ...
        for (int i=1; i<locoCount; i++) {
            ...
            if (locoDir == leadDir) {
                proto->setDirection(tChar, loco, direction);
            } else {
                proto->setDirection(tChar, loco, (direction==Reverse)?Forward:Reverse);
            }
        }
...
void ThrottleManager::changeNumberOfThrottles(bool increase) {
    if (increase) {
        maxThrottles++; if (maxThrottles>6) maxThrottles = 6;
    } else {
