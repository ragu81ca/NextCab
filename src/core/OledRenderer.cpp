	...
	clearArray();
		if (wiThrottleProtocol.getNumberOfLocomotives(currentThrottleIndexChar) > 0 ) {
			...
			for (int i=0; i<10; i++) {
				...
				if (k < MAX_FUNCTIONS) {
					j = (i<5) ? i : i+1;
					oledText[j] = String(i) + ": " + ((k<10) ? functionLabels[currentThrottleIndex][k].substring(0,10) : String(k) + "-" + functionLabels[currentThrottleIndex][k].substring(0,7));
...
void OledRenderer::renderFunctions() {
	...
	for (int i=0; i < MAX_FUNCTIONS; i++) if (functionStates[currentThrottleIndex][i]) { display.drawRBox(i*4+12,12+1,5,7,2); display.setDrawColor(0); display.setFont(FONT_FUNCTION_INDICATORS); display.drawUTF8( i*4+1+12, 18+1, String( (i<10) ? i : ((i<20) ? i-10 : i-20)).c_str()); display.setDrawColor(1); }
}
...
void OledRenderer::renderHeartbeatCheck() {
	menuIsShowing = false; RenderModel model; buildHeartbeatRenderModel(model, uiState, heartbeatCheckEnabled); for (int i=0;i<18;i++){ oledText[i] = model.lines[i]; oledTextInvert[i] = model.invert[i]; } renderArrayInternal(false,false,model.sendBuffer,model.drawTopLine);
}
...
void OledRenderer::renderAllLocos(bool hideLeadLoco) {
	lastOledScreen = last_oled_screen_all_locos; lastOledBoolParameter = hideLeadLoco; int startAt = (hideLeadLoco) ? 1 : 0; String loco; int j=0; int i=0; if (wiThrottleProtocol.getNumberOfLocomotives(currentThrottleIndexChar) > 0) { for (int index=0; ((index < wiThrottleProtocol.getNumberOfLocomotives(currentThrottleIndexChar)) && (i < 8)); index++) { j = (i<4) ? i : i+2; loco = wiThrottleProtocol.getLocomotiveAtPosition(currentThrottleIndexChar, index); if (i>=startAt) { oledText[j+1] = String(i) + ": " + loco; if (wiThrottleProtocol.getDirection(currentThrottleIndexChar, loco) == Reverse) oledTextInvert[j+1] = true; } i++; } }
}

void OledRenderer::renderMenu(const String &soFar, bool primeMenu) {
	lastOledStringParameter = soFar; int offset = 0; lastOledScreen = last_oled_screen_menu; if (!primeMenu) { offset = 10; lastOledScreen = last_oled_screen_extra_submenu; } menuIsShowing = true; bool drawTopLine = false; if ( (soFar == "") || ((!primeMenu) && (soFar.length()==1)) ) { clearArray(); int j=0; for (int i=1+offset; i<10+offset; i++) { j = (i<6+offset) ? i-offset : i+1-offset; oledText[j-1] = String(i-offset) + ": " + menuText[i][0]; } oledText[10] = "0: " + menuText[0+offset][0]; setMenuTextForOled(menu_cancel); renderArrayInternal(false,false,true,false); } else { int cmd = menuCommand.substring(0,1).toInt(); clearArray(); oledText[0] = ">> " + menuText[cmd][0] + ":"; oledText[6] = menuCommand.substring(1, menuCommand.length()); oledText[5] = menuText[cmd+offset][1]; switch (soFar.charAt(0)) { case MENU_ITEM_DROP_LOCO: { if (wiThrottleProtocol.getNumberOfLocomotives(currentThrottleIndexChar) > 0) { renderAllLocos(false); drawTopLine = true; } } case MENU_ITEM_FUNCTION: case MENU_ITEM_TOGGLE_DIRECTION: { if (wiThrottleProtocol.getNumberOfLocomotives(currentThrottleIndexChar) <= 0) { oledText[2] = MSG_THROTTLE_NUMBER + String(currentThrottleIndex+1); oledText[3] = MSG_NO_LOCO_SELECTED; setMenuTextForOled(menu_cancel); } break; } } renderArrayInternal(false,false,true,drawTopLine); }
}
...
void OledRenderer::renderSpeedStepMultiplier() { if (speedStep != currentSpeedStep[currentThrottleIndex]) { display.setDrawColor(1); display.setFont(FONT_GLYPHS); display.drawGlyph(1,38,glyph_speed_step); display.setFont(FONT_DEFAULT); display.drawStr(9,37,String(speedStepCurrentMultiplier).c_str()); } }

void OledRenderer::renderSpeed() {
	lastOledScreen = last_oled_screen_speed; menuIsShowing = false; String sLocos=""; String sSpeed=""; String sDirection=""; String sep=" "; bool foundNext=false; String nextNo=""; String nextSpeedDir=""; clearArray(); bool drawTop=false; if (wiThrottleProtocol.getNumberOfLocomotives(currentThrottleIndexChar) > 0) { for (int i=0;i<wiThrottleProtocol.getNumberOfLocomotives(currentThrottleIndexChar); i++) { sLocos += sep + getDisplayLocoString(currentThrottleIndex,i); sep = CONSIST_SPACE_BETWEEN_LOCOS; } sSpeed = String(getDisplaySpeed(currentThrottleIndex)); sDirection = (currentDirection[currentThrottleIndex]==Forward)? DIRECTION_FORWARD_TEXT : DIRECTION_REVERSE_TEXT; if (maxThrottles>1) { int nextIdx = currentThrottleIndex+1; for (int i=nextIdx;i<maxThrottles;i++) if (wiThrottleProtocol.getNumberOfLocomotives(getMultiThrottleChar(i))>0) { foundNext=true; nextIdx=i; break; } if (!foundNext && currentThrottleIndex>0) { for (int i=0;i<currentThrottleIndex;i++) if (wiThrottleProtocol.getNumberOfLocomotives(getMultiThrottleChar(i))>0) { foundNext=true; nextIdx=i; break; } } if (foundNext) { nextNo = String(nextIdx+1); int sp = getDisplaySpeed(nextIdx); nextSpeedDir = String(sp); if (currentDirection[nextIdx]==Forward) nextSpeedDir += DIRECTION_FORWARD_TEXT_SHORT; else nextSpeedDir = DIRECTION_REVERSE_TEXT_SHORT + nextSpeedDir; nextSpeedDir = String("     ") + nextSpeedDir; nextSpeedDir = nextSpeedDir.substring(nextSpeedDir.length()-5); } } oledText[0] = "   " + sLocos; drawTop=true; } else { setAppnameForOled(); oledText[2] = MSG_THROTTLE_NUMBER + String(currentThrottleIndex+1); oledText[3] = MSG_NO_LOCO_SELECTED; drawTop=true; } if (!hashShowsFunctionsInsteadOfKeyDefs) setMenuTextForOled(menu_menu); else setMenuTextForOled(menu_menu_hash_is_functions); renderArrayInternal(false,false,false,drawTop); if (wiThrottleProtocol.getNumberOfLocomotives(currentThrottleIndexChar) > 0) { renderFunctions(); display.setDrawColor(0); display.drawBox(0,0,12,16); display.setDrawColor(1); display.setFont(FONT_THROTTLE_NUMBER); display.drawStr(2,15,String(currentThrottleIndex+1).c_str()); } renderBattery(); renderSpeedStepMultiplier(); if (trackPower==PowerOn) { display.drawRBox(0,40,9,9,1); display.setDrawColor(0); } display.setFont(FONT_GLYPHS); display.drawGlyph(1,48,glyph_track_power); display.setDrawColor(1); if (!heartbeatCheckEnabled) { display.setFont(FONT_GLYPHS); display.drawGlyph(13,49,glyph_heartbeat_off); display.setDrawColor(2); display.drawLine(13,48,20,41); display.setDrawColor(1); } display.setFont(FONT_DIRECTION); display.drawStr(79,36,sDirection.c_str()); const char *cSpeed = sSpeed.c_str(); display.setFont(FONT_SPEED); int width = display.getStrWidth(cSpeed); display.drawStr(22+(55-width),45,cSpeed); if (maxThrottles>1 && foundNext) { display.setFont(FONT_NEXT_THROTTLE); display.drawStr(85+34,38,nextNo.c_str()); display.drawStr(85+12,48,nextSpeedDir.c_str()); } display.sendBuffer(); }
