// MatrixKeypad3x4Input.h — Self-contained 4×3 GPIO matrix keypad driver.
//
...
// ── Dimensions ──────────────────────────────────────────────────
static constexpr byte MK3X4_ROWS = 4;
static constexpr byte MK3X4_COLS = 3;

// ── Key map ─────────────────────────────────────────────────────
static char _mk3x4Keys[MK3X4_ROWS][MK3X4_COLS] = {
    {'1','2','3'},
    {'4','5','6'},
    {'7','8','9'},
    {'*','0','#'}
...
#ifdef USE_TFT_ESPI
  // ESP32-S3 safe GPIOs — avoid TFT SPI, USB, flash/PSRAM, UART0, strapping pins
  #ifndef KEYPAD_ROW_PINS
    static byte _mk3x4RowPins[MK3X4_ROWS]  = {38, 39, 40, 41};
  #else
    static byte _mk3x4RowPins[MK3X4_ROWS]  = KEYPAD_ROW_PINS;
  #endif
  #ifndef KEYPAD_COLUMN_PINS
    static byte _mk3x4ColPins[MK3X4_COLS]  = {42, 2, 4};
  #else
    static byte _mk3x4ColPins[MK3X4_COLS]  = KEYPAD_COLUMN_PINS;
  #endif
#else
  // Original ESP32 (lolin32_lite etc.)
  #ifndef KEYPAD_ROW_PINS
    static byte _mk3x4RowPins[MK3X4_ROWS]  = {19, 18, 17, 16};
  #else
    static byte _mk3x4RowPins[MK3X4_ROWS]  = KEYPAD_ROW_PINS;
  #endif
  #ifndef KEYPAD_COLUMN_PINS
    static byte _mk3x4ColPins[MK3X4_COLS]  = {4, 0, 2};
  #else
    static byte _mk3x4ColPins[MK3X4_COLS]  = KEYPAD_COLUMN_PINS;
  #endif
#endif
...
class MatrixKeypad3x4Input : public IInputDevice {
public:
    explicit MatrixKeypad3x4Input(IInputDevice::DispatchFn dispatchFn)
        : _dispatch(dispatchFn),
          _keypad(makeKeymap((char*)_mk3x4Keys),
                  _mk3x4RowPins, _mk3x4ColPins,
                  MK3X4_ROWS, MK3X4_COLS) {}

    void begin() override {
        _keypad.setDebounceTime(KEYPAD_DEBOUNCE_TIME);
        _keypad.setHoldTime(KEYPAD_HOLD_TIME);
        Serial.println("[MatrixKeypad3x4] Initialised");
    }
    ...
    const char* name() const override { return "MatrixKeypad3x4"; }
